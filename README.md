...
```sh
npm install git+ssh://git@github.com:rchowell/hammerhead-typescript.git
```
...
const params: Hammerhead.OAuthAuthorizeParams = {
  client_id: 'REPLACE_ME',
  redirect_uri: 'REPLACE_ME',
  response_type: 'code',
  scope: 'REPLACE_ME',
...
await client.routes.file.create({ file: fs.createReadStream('/path/to/file') });

// Or if you have the web `File` API you can pass a `File` instance:
await client.routes.file.create({ file: new File(['my bytes'], 'file') });

// You can also pass a `fetch` `Response`:
await client.routes.file.create({ file: await fetch('https://somesite/file') });

// Finally, if none of the above are convenient, you can use our `toFile` helper:
await client.routes.file.create({ file: await toFile(Buffer.from('my bytes'), 'file') });
await client.routes.file.create({ file: await toFile(new Uint8Array([0, 1, 2]), 'file') });
```
...
const response = await client.oauth
  ...
  .catch(async (err) => {
    if (err instanceof Hammerhead.APIError) {
...
1. Via the `HAMMERHEAD_LOG` environment variable
...
const client = new Hammerhead({
  logger: logger.child({ name: 'Hammerhead' }),
