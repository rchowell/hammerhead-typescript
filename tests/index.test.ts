...
import { APIPromise } from 'hammerhead-typescript/core/api-promise';

import util from 'node:util';
import Hammerhead from 'hammerhead-typescript';
import { APIUserAbortError } from 'hammerhead-typescript';
...
describe('instantiate client', () => {
  ...
  describe('logging', () => {
    ...
    beforeEach(() => {
      process.env = { ...env };
      process.env['HAMMERHEAD_LOG'] = undefined;
    });
    ...
    const forceAPIResponseForClient = async (client: Hammerhead) => {
    ...
    test('default logLevel is warn', async () => {
      const client = new Hammerhead({ webhookSignature: 'My Webhook Signature' });
    ...
    test('debug logs happen with debug env var', async () => {
      ...
      process.env['HAMMERHEAD_LOG'] = 'debug';
      const client = new Hammerhead({ logger: logger, webhookSignature: 'My Webhook Signature' });
    ...
    test('warn when env var level is invalid', async () => {
      ...
      process.env['HAMMERHEAD_LOG'] = 'not a log level';
      const client = new Hammerhead({ logger: logger, webhookSignature: 'My Webhook Signature' });
      expect(client.logLevel).toBe('warn');
      expect(warnMock).toHaveBeenCalledWith(
        'process.env[\'HAMMERHEAD_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]',
      );
    });
  ...
  describe('baseUrl', () => {
    ...
    afterEach(() => {
      process.env['HAMMERHEAD_BASE_URL'] = undefined;
    });
    ...
    test('env variable', () => {
      process.env['HAMMERHEAD_BASE_URL'] = 'https://example.com/from_env';
      const client = new Hammerhead({ webhookSignature: 'My Webhook Signature' });
      expect(client.baseURL).toEqual('https://example.com/from_env');
    });

    test('empty env variable', () => {
      process.env['HAMMERHEAD_BASE_URL'] = ''; // empty
      const client = new Hammerhead({ webhookSignature: 'My Webhook Signature' });
      expect(client.baseURL).toEqual('https://api.hammerhead.io/v1/api/');
    });

    test('blank env variable', () => {
      process.env['HAMMERHEAD_BASE_URL'] = '  '; // blank
      const client = new Hammerhead({ webhookSignature: 'My Webhook Signature' });
      expect(client.baseURL).toEqual('https://api.hammerhead.io/v1/api/');
    });
    ...
    test('in request options overridden by env variable', () => {
      process.env['HAMMERHEAD_BASE_URL'] = 'http://localhost:5000/env';
      const client = new Hammerhead({ webhookSignature: 'My Webhook Signature' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
  ...
  test('maxRetries option is correctly set', () => {
    const client = new Hammerhead({ maxRetries: 4, webhookSignature: 'My Webhook Signature' });
    expect(client.maxRetries).toEqual(4);

    // default
    const client2 = new Hammerhead({ webhookSignature: 'My Webhook Signature' });
  ...
  test('with environment variable arguments', () => {
    ...
    process.env['HAMMERHEAD_WEBHOOK_SIGNATURE'] = 'My Webhook Signature';
    const client = new Hammerhead();
  ...
  test('with overridden environment variable arguments', () => {
    // set options via env var
    process.env['HAMMERHEAD_WEBHOOK_SIGNATURE'] = 'another My Webhook Signature';
    const client = new Hammerhead({ webhookSignature: 'My Webhook Signature' });
    expect(client.webhookSignature).toBe('My Webhook Signature');
  });
});
...
describe('retries', () => {
  ...
  test('retry on 429 with retry-after', async () => {
    ...
    const client = new Hammerhead({ webhookSignature: 'My Webhook Signature', fetch: testFetch });
